#!/usr/bin/env python3
"""
build_b9.py - batch 12: URL repointing (NJ OK CO LA) pass (overrides only, no corpus change).

Reads batches/url-overrides-b12.tsv and rewrites:
...
SRC = {
    ...
    'websites-2': 'build-b11/tjjm-gym-websites-2.liquid',
    'websites-3': 'build-b11/tjjm-gym-websites-3.liquid',
}
...
def main():
    rows = list(csv.DictReader(open(p('batches/url-overrides-b12.tsv'), encoding='utf-8'), delimiter='\t'))
    ...
    os.makedirs(p('build-b12'), exist_ok=True)
    for name, body, src in [('tjjm-gym-websites-2.liquid', body2, SRC['websites-2']),
        ...
        open(p('build-b12/' + name), 'w', encoding='utf-8', newline='').write(body)

    # ---- post-condition: parse the built files back ----
    f2 = parse('build-b12/tjjm-gym-websites-2.liquid')[0]
    f3 = parse('build-b12/tjjm-gym-websites-3.liquid')[0]
    f1 = files['websites-1']
